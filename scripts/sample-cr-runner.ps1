...
.DESCRIPTION
  ...
  Di Windows PowerShell 5.1, pemanggilan otomatis dialihkan ke scripts\cr-runner-ps51.ps1 (logging yang kompatibel dengan 5.1).
...
if ($PSVersionTable.PSVersion.Major -eq 5 -and $PSVersionTable.PSVersion.Minor -eq 1) {
  $ps51Script = Join-Path $PSScriptRoot "cr-runner-ps51.ps1"
  if (-not (Test-Path -LiteralPath $ps51Script -PathType Leaf)) {
    throw "Berkas cr-runner-ps51.ps1 tidak ditemukan: $ps51Script"
  }
  & $ps51Script @PSBoundParameters
  exit 0
}
